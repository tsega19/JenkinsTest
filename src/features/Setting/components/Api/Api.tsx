...
import { ApiKeySchema, TApiKeySchema } from "../../../../lib/validator";
import {
  GenerateApiKey,
  GetAllApiKeys,
  changeApiKeyStatusApi,
  removeApiKey,
} from "../../../../Service/Merchant/ApiKey";
...
export const Api = () => {
  ...
  } = useForm<TApiKeySchema>({
    resolver: zodResolver(ApiKeySchema),
  });

  const onSubmit = async (data: TApiKeySchema) => {
    try {
      if (userToken) {
  ...
  const handelShowButton = (id: number) => {
    setShowSecreteKey(!showSecreteKey);
    setRowId(id);
  };

  return (
    <section className="w-[100%] mx-auto px-0 sm:px-6 lg:px-8">
      <Dialog
        open={dialogOpen}
        ...
        <div className="mb-10">
          <div className="mt-5">
            <div className="flex justify-end p-3">
              <button
                className="px-10 p-2 rounded-md bg-primary text-white font-bold"
                onClick={handleOpen}
              >
                Generate Api Key
              </button>
                    ...
                    <>
                      <tr className="text-left text-sm" key={index}>
                        ...
                        <td
                          ...
                          onClick={() => handelShowButton(index)}
                        >
                          {index === rowId && showSecreteKey ? (
                            api.secret
                          ) : (
                            <span className="">
                        ...
                        <td className="p-4">
                          ...
                          <button
                            onClick={() => handleClickOpen(api.id)}
            ...
            <div className="relative flex flex-col items-center  gap-4 bg-white w-[30rem] p-10 rounded-md">
              ...
              <div className="flex flex-col gap-2 w-full">
                ...
                <input
                  type="text"
                  {...register("label")}
                  className="px-2 bg-gray-100 p-2 rounded-md"
                />
                {errors.label && (
              ...
              <div className="flex flex-col gap-2 w-full">
                ...
                {errors.clientType && (
                  <span className="text-red-500">{errors.clientType.message}</span>
                )}
              </div>
              <div className="flex flex-col gap-2 w-full">
