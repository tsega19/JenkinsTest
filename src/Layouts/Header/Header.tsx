...
export const Header: React.FC = () => {
  ...
  const [linkDrop, setLinkDrop] = useState<boolean>(false);
  ...
  const handleUserItemClick = (onClick: (() => void) | undefined) => {
    if (onClick) onClick();
    setDrop(false);
  };
      ...
      <div className="container mx-auto flex justify-between items-center h-16 ">
        <div className=" w-full md:w-auto lg:block hidden">
          <div className="flex items-center gap-3 ml-4 ">
            ...
            <input
              type="text"
              placeholder="Search"
              className="w-full md:w-[30rem] py-2 focus:outline-none"
            />
          </div>
        </div>
        <div className="flex items-center justify-end w-full px-5 lg:px-0">
          <div className="flex items-center gap-5">
            ...
            />
            <div
              className="p-[4px] rounded-full bg-primary text-white cursor-pointer lg:block hidden"
              title="Create Link"
              onClick={() => setLinkDrop(!linkDrop)}
            >
              <FaPlus size={"0.8rem"} />
            </div>
          ...
          <div ref={linkDropdownRef}>
            {linkDrop && (
              <div className="absolute right-[12rem] top-14 z-50">
                <div className="bg-white p-3 shadow-xl rounded-md">
                  <ul>
                    {createLink.map((item, index) => (
                      <li
                        key={index}
                        className="flex items-center text-gray-500 gap-3 text-sm hover:bg-primary/90 hover:text-white p-2 rounded-md cursor-pointer"
                  ...
                  <ul className="flex flex-col">
                    {userLinks.map((item, index) => (
                      <li
                        key={index}
                        className="flex items-center text-gray-500 gap-3 text-sm hover:bg-primary/90 hover:text-white p-2 rounded-md cursor-pointer"
                        onClick={() => handleUserItemClick(item.onclick)}
                      >
                        <div
                          className="text-2xl"
                          onClick={() => handleUserItemClick(item.onclick)}
